...
def exp(Xtrain, ytrain, 
    ...
    for iteration in tqdm(range(1, budget + 1)):
        # evaluate the model
        f1 = evaluate_lse_model(model, likelihood, Xtest, ytest, h)
        f1_all[iteration - 1] = f1
        # sample new point
        if sample=="random":
    ...
    return file_name, f1_all

def save_result(file_name, f1_all):
...
def plot_final_result(budget, runs, data_name, h, N, n_candidates, batch, note):
    random_result = read_result(f"result/{data_name}_random_h{h}_N{N}_NC{n_candidates}_budget{budget}_batch{batch}_{note}.txt", budget, runs)
    uniform_result = read_result(f"result/{data_name}_uniform_h{h}_N{N}_NC{n_candidates}_budget{budget}_batch{batch}_{note}.txt", budget, runs)
    nearest_result = read_result(f"result/{data_name}_nearest_h{h}_N{N}_NC{n_candidates}_budget{budget}_batch{batch}_{note}.txt", budget, runs)
    straddle_result = read_result(f"result/{data_name}_straddle_h{h}_N{N}_NC{n_candidates}_budget{budget}_batch{batch}_{note}.txt", budget, runs)
    ...
    ax.legend()
    plt.savefig(f"figures/final_{data_name}_h{h}_N{N}_NC{n_candidates}_budget{budget}_batch{batch}_{note}.png")
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    X_range = {i: [0, args.max_dim, 0.1] for i in range(args.n_dim)}
    for run in range(args.runs):
        ...
        for result in run_results.values():
            file_name, f1_all = result[0], result[1]
            save_result(file_name, f1_all)

...
def batch_nearestpos(X_range, n_candidates, model, h, batch):
    X_sample, y_sample = sample_from_GP(X_range, n_candidates, model)
    lse_idx = torch.where(y_sample >= h)[0]
    if lse_idx.shape[0] > 0:
        X_sample, y_sample = X_sample[lse_idx], y_sample[lse_idx]
    selected_idx = torch.abs(y_sample - h).argsort()[:batch]
    return X_sample[selected_idx, :].reshape((batch, X_sample.shape[1]))
...
def exp(Xtrain, ytrain, 
    ...
    for iteration in tqdm(range(1, budget + 1)):
        ...
        elif sample=="nearestpos":
            X_sample_point = batch_nearestpos(X_range, n_candidates, model, h, batch)            
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_name", type=str, default="syn2dlinear", help="data name")
    ...
    for run in range(args.runs):
        ...
        run_results = {}
        for sample in ["random", "uniform", "nearest", "nearestpos", "straddle"]:
